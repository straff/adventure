...
class Places
  ...
  def build
    ...
    porch = Place.new 'porch', 'an area', :outside
    porch.west_boundary = Door.new 'heavy wooden', :closed, :unlocked
    porch.south_boundary = Window.new 'glass', :closed, :locked
    porch.east_boundary = Door.new 'glass', :closed, :locked
    porch.north_boundary = Wall.new 'wooden', 'pale green'
    locations[porch.name] = porch

    workshop = Place.new 'workshop', 'an area', :inside
    workshop.west_boundary = porch.east_boundary
    workshop.south_boundary = Door.new 'metal roller', :closed, :unlocked, :end_boundary
    workshop.east_boundary = Wall.new 'plaster', 'painted white'
    workshop.north_boundary = Window.new 'glass', :closed, :locked, :end_boundary
    locations[workshop.name] = workshop
    
    foyer = Place.new 'foyer', 'a room', :inside
    foyer.east_boundary = porch.west_boundary
    foyer.south_boundary = Way.new 'short_hall', 'short hallway'
    foyer.west_boundary = Door.new 'glass sliding', :closed, :unlocked 
    foyer.north_boundary = Way.new 'area', 'an area'
